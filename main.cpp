...
long IDLE_WARNING_SEC = 10;
long IDLE_BLACKOUT_SEC = 20;
double MAX_DIM_OPACITY = 0.65;
int USE_DPMS = 0; 
...
std::string getConfigPath() {
    ...
    return std::string(homeDir) + "/.config/idle_dimmer/config.toml";
}
...
void loadConfiguration() {
    ...
    while (std::getline(configFile, line)) {
        ...
        // Safely skip TOML section headers like [timers] or [display]
        ...
        if (std::getline(is_line, key, '=')) {
            ...
            if (std::getline(is_line, value)) {
                ...
                // Convert key to uppercase to keep the internal variables working seamlessly
                ...
                } else if (key == "USE_DPMS") {
                    USE_DPMS = std::stoi(value);
                }
            }
        }
    }
    std::cout << "[Config] Loaded TOML Settings -> Warning: " << IDLE_WARNING_SEC
              << "s | Blackout: " << IDLE_BLACKOUT_SEC << "s | Max Dimness: "
              << (MAX_DIM_OPACITY * 100.0) << "% | USE_DPMS: " << USE_DPMS << "\n";
}
        ...
        if (idleTime >= idleBlackoutMs) {
            if (currentState != STATE_BLACKOUT) {
                ...
                if (USE_DPMS == 1) {
                    ...
                    std::cout << "[Action] Targets hit. Simulating blackout overlay (100% black).\n";
                }
            }
            paintOverlay(display, overlay, format, width, height, 1.0);
        }
        else if (idleTime >= idleWarningMs) {
            if (currentState == STATE_BLACKOUT && USE_DPMS == 1) {
                forceScreenWake();
            }
